...
void animate(char *msg, unsigned char *program) {
	...
	printf("test pc:%d\n",pc);
		...
		printf("0\n");
            ...
            case 0x01:
		if (arg1 >= 16) {
		    printf("This is invalid arg1 that surpasses number of virtual register(16)!\n");
		    exit(0);
		}
                regs[arg1] = *mptr;
		printf("1\n");
		...
		printf("3\n");
            ...
            case 0x04:
		if (arg2 >= 16) {
		    printf("This is invalid arg2 that surpasses number of virtual register(16)!\n");
		    exit(0);
		}
                regs[arg2] = arg1;
		printf("4\n");
...
struct this_gift_card *gift_card_reader(FILE *input_fd) {
	...
	while (!feof(input_fd)) {
		...
		if (ret_val->num_bytes < 0) {
			printf("The number of bytes of this_gift_card listed in the file shouldn't be negative.\nPlease consider rewriting the gift card metadata\n");
			exit(0);
		}
